...
smooth_factor = 3 # higher is smoother but more lag
...
while cap.isOpened():
    ...
    if result.hand_landmarks:
        for i in range(len(result.hand_landmarks)):
            ...
            screen_y = np.interp(iy, [80, h - 80], [0, screen_h])

            curr_x = prev_x + (screen_x - prev_x) / smooth_factor
            curr_y = prev_y + (screen_y - prev_y) / smooth_factor
            curr_x = max(0, min(screen_w, curr_x))
            curr_y = max(0, min(screen_h, curr_y))
