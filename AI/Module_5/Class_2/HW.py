...
def daylight_edition(image):
    """Bright, soft, like daylight ☀️"""
    image = ImageEnhance.Brightness(image).enhance(1.5)
    image = ImageEnhance.Contrast(image).enhance(1.1)
    return image.filter(ImageFilter.GaussianBlur(radius=2))

def night_mood(image):
    """Dark, dramatic, night-like 🌙"""
    ...
    while True:
        ...
        try:
            ...
            if save_option == 'yes':
                ...
                # Make and show Daylight and Night versions
                day_img = daylight_edition(image)
                night_img = night_mood(image)

                day_img.show()    # ← add this
                night_img.show()  # ← add this

                day_img.save(f"{file_name}_daylight.png")
                night_img.save(f"{file_name}_night.png")
                print(f"📁 {file_name}_daylight.png  ← Daylight Edition")
                print(f"📁 {file_name}_night.png     ← Night Mood\n")
