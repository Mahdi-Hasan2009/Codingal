...
def reinforcement_learning_activity():
    ...
    refinement_prompt = ( ###
        f"You previously answered this prompt:\n\"{prompt}\"\n\n" ###
        f"Your previous response was:\n\"{initial_response}\"\n\n" ###
        f"The user rated that response {rating}/5 and gave this feedback:\n" ###
        f"\"{feedback}\"\n\n" ###
        f"Please rewrite your response, taking the feedback into account. " ###
        f"Only output the improved response, with no extra preamble." ###
    ) ###
    improved_response = generate_response(refinement_prompt, temperature=0.3, max_tokens=1024) ###
